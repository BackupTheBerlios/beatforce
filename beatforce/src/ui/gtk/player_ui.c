...
void PLAYERUI_CreateWindow(int nr,void *window)
{
    ...
    UI_Players[nr].Normal.ButtonPlay= gtk_button_new_with_label ("Play");

    /* This packs the button into the window (a gtk container). */
//    gtk_container_add (GTK_CONTAINER (window), UI_Players[nr].Normal.ButtonPlay);

//    gtk_signal_connect(G_OBJECT(UI_Players[nr].Normal.ButtonPlay),"clicked",
//                       G_CALLBACK(PLAYERUI_PlayButton),&UI_Players[nr]);
    gtk_widget_show(UI_Players[nr].Normal.ButtonPlay);
    ...
    UI_Players[nr].Normal.ButtonPause=gtk_button_new_with_label ("Pause");
#if 0
    if(Button->normal)
...
//    GtkWidgetHide(UI_Players[nr].Normal.ButtonPause);
